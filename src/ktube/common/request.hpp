...
inline std::string GetRequestError(cpr::ErrorCode code) {
  ...
  // if (code == cpr::ErrorCode::REQUEST_CANCELLED)   // Unsupposed on some versions
...
nlohmann::json json() const {
  const bool JSON__DO_NOT_THROW{false};
  return nlohmann::json::parse(response.text, nullptr, JSON__DO_NOT_THROW);
}
